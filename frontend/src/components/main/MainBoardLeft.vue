    ...
    <div class="comunity-group">
      <div class="comunity-header">
        <div class="comunity-header-text">
          <router-link to="/board">활발한 이야기 TOP 5 💬 </router-link>
        </div>
      </div>
      <div
        v-for="board in boardReplyRank"
        :key="board.boardId"
...
import { ref } from "vue";
const boardReplyRank = ref({});

const getBoard = async () => {
  const response = await ncapi.get("/api/board/reply-rank");
  boardReplyRank.value = response.data;
  console.log(response);
  console.log(boardReplyRank.value);
};
onMounted(() => {
  getBoard();
});
